...
with tab1:
    ...
    if submit_button and flag == 0:
        ...
        with open(r"C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\model.pkl", 'rb') as file:
            loaded_model = pickle.load(file)
        with open(r'C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\scaler.pkl', 'rb') as f:
            scaler_loaded = pickle.load(f)
        with open(r"C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\t.pkl", 'rb') as f:
            t_loaded = pickle.load(f)
        with open(r"C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\s.pkl", 'rb') as f:
...
with tab2:
    ...
    if csubmit_button and cflag == 0:
        ...
        with open(r"C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\cmodel.pkl", 'rb') as file:
            cloaded_model = pickle.load(file)
        with open(r'C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\cscaler.pkl', 'rb') as f:
            cscaler_loaded = pickle.load(f)
        with open(r"C:\\Users\\Gayathri\\OneDrive\\Desktop\\prj5\\ct.pkl", 'rb') as f:
            ct_loaded = pickle.load(f)
        # Predict the status for a new sample
        # 'quantity tons_log', 'selling_price_log','application', 'thickness_log', 'width','country','customer','product_ref']].values, X_ohe
        new_sample = np.array([[int(ccustomer),ccountry,
                                capplication,float(cwidth),
                                int(product_ref),np.log(float(cselling)),
                                np.log(float(cquantity_tons)),
                                np.log(float(cthickness)),
                                citem_type]])
        new_sample_ohe = ct_loaded.transform(new_sample[:, [8]]).toarray()
        new_sample = np.concatenate((new_sample[:, [0, 1, 2, 3, 4, 5, 6, 7]], new_sample_ohe), axis=1)
        new_sample = cscaler_loaded.transform(new_sample)
        new_pred = cloaded_model.predict(new_sample)
        st.write("Predicted Status:", new_pred)
        if new_pred == 1:
